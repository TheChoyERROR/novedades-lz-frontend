...
export function ThemeToggle({
  ...
  useEffect(() => {
    const stored = localStorage.getItem('novedades-theme');
    ...
    localStorage.setItem('novedades-theme', resolved);
    setMounted(true);
  }, []);
  ...
  const toggleTheme = () => {
    ...
    localStorage.setItem('novedades-theme', next);
  };
  ...
  if (!mounted) {
    return (
      ...
      >
        <span className="flex h-8 w-8 items-center justify-center rounded-full bg-surface-muted text-primary-600">
